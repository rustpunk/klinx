...
- **Verified:** Source `correlation_key` fields and post-Aggregate failure-grain fields are distinct canvas roles. `FieldRow::is_correlation_key` is reserved for source-declared CK drivers and carried passthrough copies; Aggregate `group_by` output fields use `FieldRow::is_aggregate_grain` so fields like `invoice_date` can be styled as the grouped failure grain without being mislabeled as source CK. Hidden `$ck.aggregate.*` bridge fields remain filtered from normal rows.
