...
- **Verified:** Field edges carry an INDIRECT *influence* class alongside the DIRECT value class (#147). `FieldEdgeKind` is `Passthrough`/`Access`/`Derive` (DIRECT) plus `Filter`/`GroupBy`/`JoinKey`/`Conditional` (INDIRECT, OpenLineage subtypes). `EdgeNature` is derived purely from the kind via `FieldEdgeKind::nature()` — never stored — so an illegal Direct+JoinKey state cannot exist. INDIRECT edges connect a Cull/Route predicate's support producer to every surviving output row (deduped on the full `(from,from_field,to,to_field,kind)` tuple), tag a Merge/Combine shared fan-in carry as `JoinKey`, and represent the Aggregate grain as the `GroupBy` edge. Both lineage paths (`compute_field_lineage` and `resolved_pipeline_field_lineage`) emit them via shared helpers. Evidence: `pipeline_view` Cull/Route/Merge/Aggregate INDIRECT fixtures and `field_lineage` `nature`/`predicate_support` tests.
