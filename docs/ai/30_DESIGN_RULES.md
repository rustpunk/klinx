...
- **Verified:** Field lineage should use clean CXL parses; parse errors render diagnostics/rows but should not drive inferred edges. This includes control-flow *predicates*: a `Cull` `drop_group_when` or `Route` branch condition that fails `field_lineage::predicate_support` (a parse failure) infers NO INDIRECT edges — never lineage from unparseable CXL (#147).
- **Verified:** Field edges carry an INDIRECT *influence* class alongside the DIRECT value class (#147). `FieldEdgeKind` is `Passthrough`/`Access`/`Derive` (DIRECT) plus `Filter`/`GroupBy`/`JoinKey`/`Conditional` (INDIRECT, OpenLineage subtypes). `EdgeNature` is derived purely from the kind via `FieldEdgeKind::nature()` — never stored — so an illegal Direct+JoinKey state cannot exist. INDIRECT edges connect a Cull/Route/Combine predicate's support producer to every surviving output row (deduped on the full `(from,from_field,to,to_field,kind)` tuple), derive a Combine `JoinKey` from its `where_expr` (a Merge is a streamwise row UNION — `MergeMode::Concat`/`Interleave` — that performs no join and emits NO `JoinKey`), and represent the Aggregate grain as the `GroupBy` edge. All influence kinds flow through the one `node_influence_predicates` → `predicate_support` path, so a Combine join key whose two sides differ in name (`left.k1 == right.k2`) is captured and an unrelated same-named column is not. Both lineage paths (`compute_field_lineage` and `resolved_pipeline_field_lineage`) emit them via shared helpers. Evidence: `pipeline_view` Cull/Route/Combine/Aggregate INDIRECT fixtures (including the Merge-is-a-union and where_expr-not-name-collision cases) and `field_lineage` `nature`/`predicate_support` tests.
- **Verified:** Every `FieldEdge` and `FieldRow` carries a graded `Precision` tier — `Exact`/`Approximate`/`Unknown` — orthogonal to `FieldEdgeKind` and NEVER folded into it (#148). Precision is EXCLUDED from edge identity: `FieldEdge`'s hand-written `PartialEq` and the `EdgeAccumulator` dedup key are the same 5-tuple, so a value carry and its precision annotation never split one edge. Classification is centralized in the `FieldEdge::carry`/`derive`/`influence`/`conservative_fan_in` constructors (Exact = identity carry or clean fully-resolved derive; Approximate = any INDIRECT edge, an `emit each`-fanned derive, or a CXL-less Merge fan-in; `Unknown` lives on a `FieldRow` whose node's CXL failed `parse_clean`). Row precision is the worst of the edges producing it (`derive_row_precision` folds the `to` endpoint), defaulting Exact. Both lineage paths emit through the shared `emit_group_by_edges`/`push_direct`/`push_deduped` surface. Evidence: `field_lineage` precision-constructor + `worst`/slug tests and `pipeline_view` per-tier edge+row fixtures.
...
- **Verified:** Canvas node display modes are panel-owned projection state. `components/canvas/panel.rs` resolves global/per-node/zoom-dependent modes, applies field ranking/caps/temporary reveal rows before connector anchor resolution, and passes the projected `StageView` to `CanvasNode`. `CanvasNode` should render the supplied rows and emit display-mode actions, not independently hide field rows after anchors and card heights have been computed.
- **Verified:** Field connector colors are CSS-class driven; do not inline strokes casually. DIRECT field cables render solid; INDIRECT influence cables (#147) render ghosted + dashed via a `--indirect` modifier (driven by `FieldEdgeKind::nature()`) plus a per-kind hue class (`--filter`/`--groupby`/`--joinkey`/`--conditional`). INDIRECT cables stay collapsed-by-default — they draw only when a field is selected, through the existing hover/pin reveal gating (no always-on overlay).
- **Verified:** The post-Aggregate group grain is represented exactly once, as the `GroupBy` INDIRECT field edge — not a `FieldRow` flag (#147 retired `FieldRow::is_aggregate_grain`). `FieldRow::is_correlation_key` remains the distinct source-CK role (source-declared drivers and carried passthrough copies). Readers that need the grain derive it from incident `GroupBy` edges: the inspector "aggregate grain" badge (`is_group_by_grain`) and the canvas preview-rank/search grain set (`FieldRankSignals::aggregate_grain_by_node`). Hidden `$ck.aggregate.*` bridge fields remain filtered from normal rows.
- **Verified:** Lineage `Precision` (#148) is surfaced, not hidden. The Inspector shows a per-field precision badge (worst of the field's row precision and its incident trace-hop precisions) and a per-hop precision badge on each trace row; the edgeless empty-state keeps the preserved "No field-level lineage edges mention this field…" message. The canvas shows a hatched node-corner (`.klinx-node-precision-corner`, `data-precision` hue, node's worst field precision) revealed ONLY on selection/hover via CSS — no always-on overlay (avoid badge fatigue). Evidence: `components/inspector/model.rs` precision/empty-state tests and `components/canvas/panel.rs` `precision_corner_css_reveals_only_on_selection_or_hover`.
- **Verified:** The visible canvas requests `CanvasLayoutEngine::PortAwareSugiyama` and falls back to the current barycenter view when stage, branch, or field anchors cannot be validated. The port-aware ranker keeps every edge left-to-right while placing sources immediately before their earliest consumers instead of forcing a source-only column; same-rank ordering uses bounded predecessor/successor sweeps with semantic port scores.
