...
- **Layout migration boundary**: `layout_model` can represent ordered node, field-row, route-branch, and cull side-output ports with ranked layers and connector paths, but it is not yet the default renderer layout. Its current port-ordering pass is deliberately bounded: field-row ports may be reordered by incident-edge barycenter to reduce crossings, while node ports, Route condition branch declaration order, the default branch identity, and Cull side-output identity remain stable. Each layout port keeps a `StageView` anchor mapping so the future renderer can migrate without losing the existing `port_in`/`port_out`, field-row, and branch-row anchors. Prior research in `docs/research/2026-06-13-field-lineage-ui.md` and `docs/research/2026-06-14-route-node-visualization.md` points toward a Rust Sugiyama-style layered pass with port-aware crossing minimization and orthogonal routing; this scaffold captures the data shape before the visible canvas migration.
...
- Parsing/view: `sync::{parse_yaml, try_parse_yaml, parse_composition}`, `pipeline_view::{derive_pipeline_view, derive_resolved_pipeline_view, derive_composition_view, derive_body_view, derive_partial_pipeline_view, layout_model}`.
