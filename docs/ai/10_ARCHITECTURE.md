...
- **Desktop-only boundary**: no wasm/web target or Playwright browser target exists today. The planned browser architecture (Rust core and Axum server, React UI, Tauri desktop) is recorded in [decisions/0001-browser-ui-architecture.md](decisions/0001-browser-ui-architecture.md).
