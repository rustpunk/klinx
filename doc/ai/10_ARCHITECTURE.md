...
6. `pipeline_view.rs` and `pipeline_view/field_lineage.rs` derive canvas-ready models, layout, connections, branch ports, and field lineage.
...
- Parsing/view: `sync::{parse_yaml, try_parse_yaml, parse_composition}`, `pipeline_view::{derive_pipeline_view, derive_composition_view, derive_partial_pipeline_view}`.
